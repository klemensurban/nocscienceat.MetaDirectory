...
namespace nocscienceat.MetaDirectory.Services.UserSyncService
{
    ...
    public class UserSyncService : IUserSyncService
    {
        ...
        private abstract record ActionClassifications
        {
            private ActionClassifications() { }
            public sealed record IgnoreSamAccountName() : ActionClassifications;
            public sealed record NoSapPersNr() : ActionClassifications;
            public sealed record DuplicateSapPersNr() : ActionClassifications;
        }
        ...
        public async Task SyncUsersAsync()
        {
            try
            {
                ...
                for (int i = adUsers.Count - 1; i >= 0; i--)
                {
                    ...
                    ActionClassifications? action = adUser switch
                    {
                        _ when !string.IsNullOrWhiteSpace(adUser.SamAccountName) && samAccountNamesToIgnore.Contains(adUser.SamAccountName!) => new ActionClassifications.IgnoreSamAccountName(),
                        _ when string.IsNullOrWhiteSpace(adUser.SapPersNr) => new ActionClassifications.NoSapPersNr(),
                        _ when !string.IsNullOrWhiteSpace(adUser.SapPersNr) && adUser.SapPersNr!.Trim().ToUpperInvariant() == "NOSAP" => new ActionClassifications.NoSapPersNr(),
                        _ when !string.IsNullOrWhiteSpace(adUser.SapPersNr) && !seenSapPersNr.Add(adUser.SapPersNr!) => new ActionClassifications.DuplicateSapPersNr(),
                        _ => null
                    ...
                    switch (action)
                    {
                        case ActionClassifications.IgnoreSamAccountName:
                            _logger.LogDebug(
                                "AD user with SamAccountName in ignore list will be ignored: {SamAccountName}, DistinguishedName: {DistinguishedName}",
                                adUser.SamAccountName, adUser.DistinguishedName);
                            adUsers.RemoveAt(i);
                            break;
                        case ActionClassifications.NoSapPersNr:
                            _logger.LogWarning(
                                "AD user without SapPersNr will be ignored: {SamAccountName}, DistinguishedName: {DistinguishedName}",
                                adUser.SamAccountName, adUser.DistinguishedName);
                            adUsers.RemoveAt(i);
                            break;
                        case ActionClassifications.DuplicateSapPersNr:
                            hasDuplicateSapPersNr = true;
                            _logger.LogError(
                                "Duplicate SapPersNr found in AD users: {SapPersNr}, SamAccountName: {SamAccountName}, DistinguishedName: {DistinguishedName}",
                                adUser.SapPersNr, adUser.SamAccountName, adUser.DistinguishedName);
                            break;
                    }
                }
                ...
                CudManager<string, IdmUser, AdUser> userCudManager = new(new UserCudDataAdapter(_userSyncServiceSettings.CudAdapter, _logger), idmUsers, adUsers)
                ...
                {
                    _logger.LogWarning(
                        "AD user not found in IDM: {SamAccountName}, DistinguishedName: {DistinguishedName}, SapPersNr: {SapPersNr}",
                        item.SamAccountName, item.DistinguishedName, item.SapPersNr);
                }
                ...
                foreach (IdmUser? item in userCudManager.Items2Create)
                {
                    if (sapPersNumbersToIgnore.Contains(item.SapPersNr))
                        _logger.LogDebug(
                            "IDM user not found in specified AD OUs: {Sn} {GivenName}, SapPersNr: {SapPersNr}", item.Sn,
                            item.GivenName, item.SapPersNr);
                    else
                        _logger.LogWarning(
                            "IDM user not found in specified AD OUs: {Sn} {GivenName}, SapPersNr: {SapPersNr}", item.Sn,
                            item.GivenName, item.SapPersNr);
            ...
            {
                _logger.LogError(ex, "An error occurred during user synchronization.");
            }
        }
    }
}
